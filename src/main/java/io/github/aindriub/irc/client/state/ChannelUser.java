 ...
 *
 * <p>{@link #equals(Object)} and {@link #hashCode()} compare nicks using Java's
 * locale-independent case-insensitive comparison ({@code equalsIgnoreCase} /
 * {@code toLowerCase(Locale.ROOT)}), not the server's {@link CaseMapping}: this
 * class is not how {@link ChannelState} looks users up (that uses the server's
 * mapping), it is just a value the caller may want to compare or put in a set.
 */
